...
$K$-Nearest Neighbors (K-NN) is a non-linear classification method. Given a positive integer $K$, the K-NN classifier identifies the $K$ observations (represented the set as $N_0$) nearest to an observation $x_0$. It then estimates the conditional probability for category $j$ as 
...
The observation $x_0$ assign to the category $j$ with the largest conditional probability. Here are the steps of the K-NN classifier.

* Step 1: Choose the number $K$ of neighbors.

* Step 2: Take the $K$ nearest neighbors of the new observation $x_0$ using the distance measure (euclidean distance etc.)
